    ...
    private void launch()
    {
        try {
            ...
            while(true) {
                ...
                String requestMessage = new String(request.getData(), "UTF-8");
                System.out.println(requestMessage);

                if(participant == null) {
                    participant = new Participant(requestMessage, request.getAddress(), request.getPort());
                    System.out.println("inialize");
                }
                else{
                    participant.setAnswer("12345");
                    System.out.println("requestMessage");
                }

                replyMessage = quiz.get(i).getTestQuestion();
                reply = new DatagramPacket(replyMessage.getBytes(), replyMessage.getBytes().length, request.getAddress(), request.getPort());
                socket.send(reply);

                //condition of summarizing
                if(++i > quiz.size()) {
                    System.out.println(participant.analize(quiz));
                }
