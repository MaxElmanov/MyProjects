...
//                .filter(x -> (x.length() % 2) != 0);
    ...
    public List<String> answer42_f(List<String> myStrings) {
        System.out.println("_f");
        myStrings = myStrings.stream()
                .map(s -> s.toUpperCase())
...
//        return words.stream().map(x -> x.charAt(x.length() / 2)).toString();
        StringBuilder sb = new StringBuilder(words.size());
        words.forEach((String s) -> sb.append(s.charAt(s.length() / 2)).toString());
        return sb.toString();
    }
    ...
    public long countWords_f() throws Exception {
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(THIS_FILE))) {
            return reader.lines()
                    .map(s -> s.split(WORD_DELIMITERS).length)
